...
const PENDING_FILE = '/Users/claw/Documents/OpenClawData/agents/taibai/workspace/state/pending-sku-upsert.json';
...
function getConfig() {
  const cfg = loadJson('openclaw.json');
  const sec = loadJson('secrets.local.json');
  return {
...
async function main() {
  ...
  if (!mode || !['propose', 'apply'].includes(mode)) {
    console.error('usage: node scripts/feishu_trade_bitable_upsert.js <propose|apply> <request-json>');
    process.exit(1);
  }
  ...
  if (mode === 'propose') {
    ...
    fs.writeFileSync(PENDING_FILE, JSON.stringify(proposal, null, 2));
  ...
  const proposal = loadJson(PENDING_FILE);
  let record;
