...
def tenant_token():
    cfg = load_json("openclaw.json")
    sec = load_json("secrets.local.json")
    payload = json.dumps({
...
def main():
    ...
    local = load_json("/Users/claw/Documents/OpenClawData/agents/taibai/workspace/trade-assets/cleaning-wringer-cart-20260105/sku-master.json")
    local_map = {item["skuId"]: item for item in local["items"]}
    ...
    with zipfile.ZipFile(args.template) as zin:
        ...
        if args.output:
            ...
            out_path = Path(f"/Users/claw/Documents/OpenClawData/agents/taibai/workspace/quotations/{ymd}-{slug}-quotation.xlsx")
